...
const App: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'course' | 'code' | 'quiz'>('course');
  const [arMode, setArMode] = useState(true);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const mode = params.get('mode');
    if (mode === 'code' || mode === 'quiz') {
      setViewMode(mode);
    }
  }, []);
  ...
  const allSlidesFlat = useMemo(
    () =>
      COURSE_CONTENT.flatMap((mod, mIdx) =>
  ...
  const currentGlobalIndex = useMemo(() => {
    let count = 0;
    for (let i = 0; i < currentModuleIndex; i += 1) {
  ...
  const progressPercentage =
    ((currentGlobalIndex + 1) / Math.max(allSlidesFlat.length, 1)) * 100;
    ...
    <div className="flex flex-col h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30 relative overflow-hidden">
      {arMode && (
        <div className="absolute inset-0 pointer-events-none z-0">
          <div
            className="absolute inset-0 bg-grid-animated opacity-10"
            style={{
              transform:
      ...
      <header className="flex-none h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-1 bg-slate-800/50 p-1 rounded-lg border border-slate-700/50">
          ...
          <button
            ...
            className={`hidden sm:flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
              viewMode === 'quiz'
                ? 'bg-purple-900/40 text-purple-300 shadow-sm'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Layers size={16} /> Quizzes
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold tracking-wider transition-all ${
              arMode
      ...
      <div className="flex flex-1 overflow-hidden z-10">
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-900/95 backdrop-blur border-r border-slate-800 transform transition-transform duration-300 ease-in-out ${
            sidebarOpen ? 'translate-x-0' : '-translate-x-full'
          } lg:relative lg:translate-x-0 flex flex-col pt-16 lg:pt-0`}
        >
          <div className="lg:mt-4 flex-1 overflow-y-auto custom-scrollbar px-4 pb-6">
            <div className="mb-6">
              <div className="text-xs font-mono uppercase tracking-[0.2em] text-slate-500 mb-2">
                Modules
              </div>
            </div>
              ...
              <div key={module.id} className="mb-6">
                ...
                <ul className="space-y-0.5">
                  {module.slides.map((slide, sIdx) => {
                    const isActive =
                      mIdx === currentModuleIndex && sIdx === currentSlideIndex;
                    return (
                      <li key={slide.id}>
                        <button
                          onClick={() => {
          ...
          {viewMode === 'course' && (
            <div className="flex-none bg-slate-900/80 backdrop-blur border-t border-slate-800 p-4 lg:px-12 z-20">
              ...
              <div className="flex items-center justify-between max-w-7xl mx-auto">
                ...
                >
                  <span>
                    {currentGlobalIndex === allSlidesFlat.length - 1
                      ? 'FINISH'
                      : 'NEXT STEP'}
                  </span>
                  <ChevronRight
